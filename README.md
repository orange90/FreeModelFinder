...
在没有 FreeModelFinder 之前，想白嫖免费大模型的用户通常会遇到这些痛点：

- **免费模型不知道去哪找**：各家 Provider 的免费策略散落在文档、博客、社区帖子里，且经常变动；分不清哪些是真免费、哪些是"赠金试用"、哪些只是限时活动。FreeModelFinder 把 OpenRouter、Gemini、Zhipu、SiliconFlow、ModelScope、NVIDIA NIM、GitHub Models、Cohere、Hugging Face、SenseNova 等 Provider 的免费文本模型统一汇总到一个目录里，并把免费判定规则写在明面上。
- **单一免费额度太少,用完就废**:免费模型普遍有每日/每分钟请求上限,单独用一个 Key 很快撞墙。通过内置的自动路由器(`auto`)与多 Provider 目录,可以在多个免费来源之间接力,一个模型用完额度自动切换到下一个,把可用时长拉长。
- **想在多客户端/多设备共享同一批免费模型**:本地网关一次配置,浏览器 UI、CLI、以及任何兼容 OpenAI/Anthropic/Gemini 协议的第三方客户端都可以复用同一份 Provider Key 与路由策略,不必在每个工具里重复填 Key。
...
1. 首次打开会进入快速向导，选择 OpenRouter（推荐）或 Gemini。
2. 粘贴 API Key，或显式确认导入当前进程中已存在的环境变量；系统会自动同步免费模型、选择默认模型并发送一次最小测试请求。
3. 在“测试”中对话；也可以再添加一个 Provider，自动开启额度用完接力。
4. 把下面的兼容 Base URL 填入其他客户端，模型填写 `auto` 即可使用自动路由。

向导只报告受支持环境变量是否存在，原始值不会返回浏览器，也不会自动导入。Key 仍使用下文所述的本机加密配置保存。选择“稍后配置”后，可以从空目录页重新进入向导，或前往“设置”配置其他 Provider。
...
需要提醒的是，免费开放的模型通常不是市面上性能最强的旗舰款，甚至有不少是发布已经超过一年的老型号。但对于新闻总结、摘要提取、简单问答、文本翻译、代码片段解释、日常聊天等相对简单的任务，它们的能力普遍是够用的。如果你的场景是复杂推理、长上下文分析、Agent 工具编排或生产环境高并发，请再评估付费旗舰模型是否更合适。
